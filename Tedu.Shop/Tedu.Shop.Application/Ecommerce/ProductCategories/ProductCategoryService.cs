...
using Tedu.Shop.Utilities.Helpers;
        ...
        IProductCategoryService
{
    ...
    {
        var query = _productCategoryRepository.GetAll().AsQueryable();
        ...
        query = query.Where(x => x.Name.Contains(keyword) || x.Description.Contains(keyword)).OrderBy(x => x.ParentId);
        return _mapper.ProjectTo<ProductCategoryViewModel>(query).ToList();
    }
